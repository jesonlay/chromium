...
constexpr SkColorType kColorType = kBGRA_8888_SkColorType;
...
void WaylandDataDevice::RequestSelectionData(const std::string& mime_type) {
  ...
  read_from_fd_closure_ =
      base::BindOnce(&WaylandDataDevice::ReadClipboardData,
...
void WaylandDataDevice::RequestDragData(
  ...
  // otherwise read(..) can block awaiting data to be sent to pipe.
  read_from_fd_closure_ =
      base::BindOnce(&WaylandDataDevice::ReadDragData, base::Unretained(this),
                     std::move(fd), mime_type, std::move(callback));
  RegisterSyncCallback();
}
...
void WaylandDataDevice::ReadDataFromFD(std::string* contents,
                                       base::ScopedFD fd,
                                       const std::string& mime_type) {
  DCHECK(contents);
...
void WaylandDataDevice::ReadClipboardData(base::ScopedFD fd,
                                          const std::string& mime_type) {
  std::string contents;
  ReadDataFromFD(&contents, std::move(fd), mime_type);
  connection_->SetClipboardData(contents, mime_type);
}

void WaylandDataDevice::ReadDragData(
    base::ScopedFD fd,
    const std::string& mime_type,
    base::OnceCallback<void(const std::string&)> callback) {
  std::string contents;
...
// TODO(jkim): Make it common API as it's duplicate of
// WaylandCursor::CreateSHMBuffer.
bool WaylandDataDevice::CreateSHMBuffer(const gfx::Size& size) {
  shm_ = connection_->shm();

  // If the requested size is same as before, SHM buffer will be reused.
  if (size == icon_buffer_size_)
    return true;

  icon_buffer_size_ = size;

  SkImageInfo info = SkImageInfo::MakeN32Premul(icon_buffer_size_.width(),
  ...
  pool.reset(wl_shm_create_pool(shm_, shared_memory_->handle().GetHandle(),
                                image_buffer_size));
  buffer_.reset(wl_shm_pool_create_buffer(
      pool.get(), 0, icon_buffer_size_.width(), icon_buffer_size_.height(),
      stride, kShmFormat));

  sk_surface_ = SkSurface::MakeRasterDirect(
      SkImageInfo::Make(icon_buffer_size_.width(), icon_buffer_size_.height(),
